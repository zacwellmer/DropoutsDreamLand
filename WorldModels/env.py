...
class CarRacingFixedNMDNRNN(CarRacingMDNRNN):
  def reset(self):
    self.rnn_states = rnn_init_state(self.rnn)
    targ_N = 289 # only generate tracks with exactly 289 tiles
    self.N_tiles = -1
    while self.N_tiles != targ_N:
        gc.collect()
        if self.with_obs:
            [z_state, obs] = super(CarRacingMDNRNN, self).reset() # calls step
            self.N_tiles = len(self.track)
...
class DreamCarRacingMDNRNN:
  def __init__(self, args):
    ...
    self.rnn_states = rnn_init_state(self.rnn)
    self.observation_space = Box(low=np.NINF, high=np.Inf, shape=(args.z_size+self.rnn.args.rnn_size*args.state_space,))
    self.action_space = args.a_width

    self._training=True # set True to use dropout

  def reset(self):
    self.rnn_states = rnn_init_state(self.rnn)
    self.rnn.inference_base.reset_dropout_mask()
    self.rnn.inference_base.reset_recurrent_dropout_mask()

    z = tf.squeeze(tf.random.normal(shape=[1, self.args.z_size]))
    self.o = z.numpy()
    h = tf.squeeze(self.rnn_states[0])
    c = tf.squeeze(self.rnn_states[1])
    if self.rnn.args.state_space == 2:
  ...
  def step(self, action):
    rnn_states_p1, z_tp1, r_tp1, d_tp1 = rnn_sim(self.rnn, self.o, self.rnn_states, action, training=self._training) # if training is True use dropout
    self.rnn_states = rnn_states_p1
    self.o = z_tp1

    z_tp1 = tf.squeeze(z_tp1)
    h = tf.squeeze(self.rnn_states[0])
    c = tf.squeeze(self.rnn_states[1])
    if self.rnn.args.state_space == 2:
        z_state = tf.concat([z_tp1, c, h], axis=-1)
    else:
        z_state = tf.concat([z_tp1, h], axis=-1)
    return z_state.numpy(), tf.squeeze(r_tp1), d_tp1.numpy(), {}
...
class DoomTakeCoverMDNRNN(DoomTakeCoverEnv):
  ...
  def _current_state(self):
    if self.rnn.args.state_space == 2:
      return np.concatenate([self.z, tf.keras.backend.flatten(self.rnn_states[1]), tf.keras.backend.flatten(self.rnn_states[0])], axis=0) 
    return np.concatenate([self.z, tf.keras.backend.flatten(self.rnn_states[0])], axis=0) # only the hidden state

  def _seed(self, seed=None):
    super(DoomTakeCoverMDNRNN, self)._seed(seed=seed)
    tf.random.set_seed(seed)
...
def make_env(args, dream_env=False, seed=-1, render_mode=False, full_episode=False, with_obs=False, load_model=True):
  ...
  elif args.env_name == 'CarRacingFixedN-v0':
    if dream_env:
      print('making dream carracing fixed n environment')
      env = DreamCarRacingMDNRNN(args=args) 
    else:
      print('making real carracing fixed n environment')
      env = CarRacingFixedNMDNRNN(args=args, full_episode=full_episode, with_obs=with_obs, load_model=load_model)
  else:
    if dream_env:
