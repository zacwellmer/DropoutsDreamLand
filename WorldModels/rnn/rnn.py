...
from tensorflow.python.keras.layers.recurrent_v2 import lstm_with_backend_selection, gpu_lstm, standard_lstm, _get_context_device_type, _runtime, _RUNTIME_UNKNOWN, _RUNTIME_CPU, _RUNTIME_GPU, _GPU_DEVICE_NAME
class DropoutLSTM(LSTM):
  '''
  Take from the r2.2 LSTM implementation (https://github.com/tensorflow/tensorflow/blob/r2.2/tensorflow/python/keras/layers/recurrent_v2.py).
  This is to force the dropout mask to be reset explicitly and to not mask action inputs
  '''
  def __init__(self, a_width, **kwargs):
    super(DropoutLSTM, self).__init__(**kwargs)
    self.a_width = a_width

    self._dropout_mask_cache = K.ContextValueCache(self._create_dropout_mask)
    self.cell._dropout_mask_cache = K.ContextValueCache(self._create_dropout_mask)

  @tf.function
  def _create_dropout_mask(self, inputs, training, count=1):
    z_masks = super(DropoutLSTM, self)._create_dropout_mask(inputs[..., :-self.a_width], training=training, count=count)
    if count == 1:
      z_masks = [z_masks]
    a_masks = [tf.ones_like(inputs[..., -self.a_width:]) for _ in range(count)] 
    masks = [tf.concat([z_masks[i], a_masks[i]], axis=-1) for i in range(count)] 
    return masks
...
class MDNRNN(tf.keras.Model):
    def __init__(self, args):
        ...
        self.inference_base = DropoutLSTM(a_width=args.a_width, units=args.rnn_size, dropout=args.rnn_input_dropout, recurrent_dropout=args.rnn_hidden_dropout, return_sequences=True, return_state=True, time_major=False)
    ...
    def get_loss(self):
        ...
        def d_loss_func(y_true, y_pred):
            ...
            y_true = tf.reshape(y_true, [batch_size, -1, 1 + 1])
    ...
    def __call__(self, inputs, training=True):
        self.inference_base.reset_dropout_mask()
        self.inference_base.reset_recurrent_dropout_mask()
...
@tf.function
def rnn_sim(rnn, z, states, a, training=True): # if we are training apply dropout masks, if not don't
